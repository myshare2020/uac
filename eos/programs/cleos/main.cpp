...
// by myextend
chain::action create_bindattrs(const name& account, uint32_t attrs) {
   return action {
      get_account_permissions(tx_permission, {config::app_account_name,config::active_name}),
      eosio::chain::bindattrs{
         .account      = account,
         .attrs        = attrs
      }
   };
...
chain::action create_bindsym(const name& account, uint32_t opr_code, uint32_t opr, uint64_t sym, 
   const name& tollor, const name& issuer, 
   uint64_t minval, uint64_t minfee, uint64_t level, uint64_t delta, uint64_t maxfee) {
   return action {
...
// by myextend
struct create_bindattrs_subcommand {
   string account;
   bool can_set_contract = false;

   create_bindattrs_subcommand(CLI::App* actionRoot) {
      auto createBind = actionRoot->add_subcommand(
                              ("bindattrs"),
                              (localized("Bind attributes on the existing account"))
      );
      createBind->add_option("account", account, localized("The name of the account"))->required();
      createBind->add_flag("--can_set_contract", can_set_contract, localized("can set contract"));

      add_standard_transaction_options(createBind, "eosio.app@active");

      createBind->set_callback([this] {
            uint32_t attrs = 0;
            if (can_set_contract)
               attrs |= (uint32_t)account_metadata_object::attrs_fields::can_set_contract;
            auto create = create_bindattrs(name(account), attrs);
            send_actions( { create } );
      });
   }
};

static uint64_t string_to_sym(std::string name) {
   if (name.size() < 1 || name.size() > 7 || 
      !all_of(name.begin(), name.end(), [](char c)->bool { return (c >= 'A' && c <= 'Z'); }))
      return 0;
...
struct create_bindsym_subcommand {
   string sym;
   string account;
   string tollor = "";
   string issuer = "";
   ...
   uint64_t minfee = 0;
   uint64_t maxfee = 0;
   bool has_tollor = false;
   bool has_issuer = false;
   bool has_fee = false;
   string opr_code;

   create_bindsym_subcommand(CLI::App* actionRoot) {
      auto createBind = actionRoot->add_subcommand(
                              ("bindsym"),
                              (localized("Bind attributes on the token symbol"))
      );
      createBind->add_option("account", account, localized("account"))->required();
      createBind->add_option("opr_code", opr_code, localized("modify,transfer"))->required();
      createBind->add_option("sym", sym, localized("token symbol"))->required();
      createBind->add_flag("--has_tollor", has_tollor, localized("has tollor"));
      createBind->add_option("--tollor", tollor, localized("tollor"));
      createBind->add_flag("--has_issuer", has_issuer, localized("has issuer"));
      createBind->add_option("--issuer", issuer, localized("issuer"));
      createBind->add_flag("--has_fee", has_fee, localized("has fee"));
      createBind->add_option("--minval", minval, localized("Need fee from the value (include)"));
      createBind->add_option("--minfee", minfee, localized("Minimal fee"));
      createBind->add_option("--level", level, localized("Fee level"));
      createBind->add_option("--delta", delta, localized("Delta fee for each level"));
      createBind->add_option("--maxfee", maxfee, localized("Maximal fee"));

      add_standard_transaction_options(createBind, "account@active");

      createBind->set_callback([this] {
            uint32_t _opr_code = 0;
            if (opr_code == "modify")
               _opr_code = (uint32_t)bindsym::opr_code::modify;
            else if (opr_code == "transfer")
               _opr_code = (uint32_t)bindsym::opr_code::transfer;
            else {
               EOSC_ASSERT(false, "invalid opr_code");
            }
            uint32_t opr = 0;
            if (has_tollor)
               opr |= (uint32_t)bindsym::opr_fields::tollor;
            if (has_issuer)
               opr |= (uint32_t)bindsym::opr_fields::issuer;
            if (has_fee)
               opr |= (uint32_t)bindsym::opr_fields::fee;
            auto create = create_bindsym(name(account),_opr_code,opr,string_to_sym(sym),
               tollor.empty() ? name() : name(tollor), 
               issuer.empty() ? name() : name(issuer), 
               minval, minfee, level, delta, maxfee);
            send_actions( { create } );
      });
   }
};

struct create_bindapp_subcommand {
   uint32_t appid = 0;
   string expire;
   string memo;
   bool has_memo = false;
   ...
   create_bindapp_subcommand(CLI::App* actionRoot) {
      auto createBind = actionRoot->add_subcommand(
                              ("bindapp"),
                              (localized("Bind attributes on the app"))
      );
      createBind->add_option("account", account, localized("account"))->required();
      createBind->add_option("opr_code", opr_code, localized("modify,transfer,create"))->required();
      createBind->add_option("appid", appid, localized("appid"));
      createBind->add_flag("--has_expire", has_expire, localized("has expire"));
      createBind->add_option("--expire", expire, localized("expire"));
      createBind->add_flag("--has_memo", has_memo, localized("has memo"));
      createBind->add_option("--memo", memo, localized("memo"));
      ...
      createBind->set_callback([this] {
            uint32_t _opr_code = 0;
            if (opr_code == "modify")
               _opr_code = (uint32_t)bindapp::opr_code::modify;
            else if (opr_code == "transfer")
               _opr_code = (uint32_t)bindapp::opr_code::transfer;
            else if (opr_code == "create")
               _opr_code = (uint32_t)bindapp::opr_code::create;
            else {
               EOSC_ASSERT(false, "invalid opr_code");
            }
            uint32_t opr = 0;
            if (has_expire)
               opr |= (uint32_t)bindapp::opr_fields::expire;
            if (has_memo)
...
// by myextend
void get_app( uint32_t appid, bool json_format ) {
   fc::variant json;
   json = call(get_app_func, fc::mutable_variant_object("appid", appid));
   auto res = json.as<eosio::chain_apis::read_only::get_app_results>();
   if (!json_format) {
      std::cout << "appid: " << res.appid << std::endl;
      std::cout << "owner: " << res.owner.to_string() << std::endl;
      std::cout << "created: " << string(res.created) << std::endl;
      std::cout << "expire: " << string(res.expire) << std::endl;
      std::cout << "memo: " << res.memo << std::endl;
   } else {
      std::cout << fc::json::to_pretty_string(json) << std::endl;      
   }
}

void get_sym( const string& sym, bool json_format ) {
   fc::variant json;
   json = call(get_sym_func, fc::mutable_variant_object("sym", sym));
   auto res = json.as<eosio::chain_apis::read_only::get_sym_results>();
   if (!json_format) {
      std::cout << "sym: " << string(res.sym) << std::endl;
      if (res.tollor.valid())
         std::cout << "tollor: " << res.tollor->to_string() << std::endl;
      if (res.issuer.valid())
         std::cout << "issuer: " << res.issuer->to_string() << std::endl;
      std::cout << "minval: " << res.minval << std::endl;
      std::cout << "minfee: " << res.minfee << std::endl;
      std::cout << "level: " << res.level << std::endl;
      std::cout << "delta: " << res.delta << std::endl;
      std::cout << "maxfee: " << res.maxfee << std::endl;   
   } else {
      std::cout << fc::json::to_pretty_string(json) << std::endl;      
   }
}
...
int main( int argc, char** argv ) {
   ...
   // get app
   uint32_t appid;
   auto getApp = get->add_subcommand("app", localized("Retrieve an app attributes from the blockchain"), false);
   getApp->add_option("appid", appid, localized("The appid of the app to retrieve"))->required();
   getApp->add_flag("--json,-j", print_json, localized("Output in JSON format") );
   getApp->set_callback([&]() { get_app(appid, print_json); });

   // get sym
   string sym;
   auto getSym = get->add_subcommand("sym", localized("Retrieve a symbol attributes from the blockchain"), false);
