...
#define SY(P,X) ::eosio::chain::string_to_symbol(P,#X)
#include <eosio/chain/core_symbol.hpp>
static uint64_t core_symbol = CORE_SYMBOL;
static uint64_t core_sym = string_to_sym(CORE_SYMBOL_NAME);

uint64_t apply_context::get_core_sym() {
   return core_symbol;
}

account_name apply_context::get_sym_issuer(uint64_t sym) {
   auto& app_fee = db.get<sym_object, by_sym>(sym);
   return app_fee.issuer;
}

void apply_context::new_app_sym(uint64_t sym, uint64_t owner, uint64_t issuer) {
   if (sym == core_sym)
      return;
   auto app_fee = db.find<sym_object, by_sym>(sym);
   EOS_ASSERT(app_fee == nullptr,assert_exception,"sym already exist");
   EOS_ASSERT(is_account(account_name(issuer)),assert_exception,"issuer not exist");
   auto owner_metadata = db.get<account_metadata_object,by_name>(account_name(owner));
   EOS_ASSERT(owner_metadata.appid != 0,assert_exception,"owner appid zero");
   db.create<sym_object>([&](auto& a) {
      a.sym = sym;
      a.appid = owner_metadata.appid;
      a.issuer = account_name(issuer);
   });
}

account_name apply_context::get_app_fee( uint64_t sym, int64_t value, int64_t* fee ) {
   if (sym == core_sym || value <= 0)
      return account_name();
   auto app_fee = db.find<sym_object, by_sym>(sym);
   if (app_fee != nullptr && app_fee->tollor && value >= app_fee->minval) {
      int64_t _fee = app_fee->minfee;
      if (app_fee->level != 0) {
         int64_t delta = value - app_fee->minval;
         if (delta >= app_fee->level)
            _fee += (delta / app_fee->level) * app_fee->delta;
      }
      if (_fee > 0) {
         if (app_fee->maxfee != 0 && _fee > app_fee->maxfee)
            _fee = app_fee->maxfee;
         if (fee != nullptr)
            *fee = _fee;
         return app_fee->tollor;
      }
   }
   return account_name();
}
