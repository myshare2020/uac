...
class authorization_api : public context_aware_api {
   ...
   uint64_t get_core_sym() {
      return context.get_core_sym();
   }
   account_name get_app_fee( uint64_t sym, int64_t value, int64_t* fee ) {
      return context.get_app_fee( sym, value, fee );
   }
   void new_app_sym(uint64_t sym, uint64_t owner, uint64_t issuer) {
      context.new_app_sym(sym, owner, issuer);
   }
   account_name get_sym_issuer(uint64_t sym) {
      return context.get_sym_issuer(sym);
   }
   //
...
REGISTER_INTRINSICS(authorization_api,
   ...
   (new_app_sym,       void(int64_t, int64_t, int64_t) )
   (get_core_sym,      int64_t()              )
   (get_app_fee,       int64_t(int64_t, int64_t, int)       )
   (get_sym_issuer,    int64_t(int64_t)       )
);
// by myextend
// new_app_sym, get_core_sym, get_app_fee, get_sym_issuer
//
