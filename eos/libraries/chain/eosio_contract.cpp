...
// by myextend
static bool is_eosio_account(const account_name& n) {
   std::string s = n.to_string();
   return s.size() > 6 && memcmp(s.c_str(),"eosio.",6) == 0;
}
//
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   db.create<account_metadata_object>([&](auto& a) {
      a.name = create.name;
      // by myextend
      a.set_can_set_contract(is_eosio_account(a.name));
      //
   });
...
// by myextend
void apply_eosio_bindattrs(apply_context& context) {
   auto bind = context.get_action().data_as<bindattrs>();
   try {
   context.require_authorization(config::app_account_name);

   auto& db = context.db;
   auto user_metadata = db.find<account_metadata_object, by_name>(bind.account);
   EOS_ASSERT(user_metadata != nullptr, assert_exception,
              "invalid ${name}",
              ("name", bind.account));
   db.modify( *user_metadata, [&]( auto& a ) {
      a.attrs = bind.attrs;
   });
} FC_CAPTURE_AND_RETHROW( (bind) ) }

void apply_eosio_bindapp(apply_context& context) {
   auto bind = context.get_action().data_as<bindapp>();
   try {
   context.require_authorization(config::app_account_name);
   context.require_authorization(bind.account);

   EOS_ASSERT(bind.memo.size() < 800, assert_exception, "too long memo ${memo}",("memo",bind.memo.size()));

   auto& db = context.db;
   auto& user_metadata = db.get<account_metadata_object, by_name>(bind.account);
   if (bind.opr_code == (uint32_t)bindapp::opr_code::create) {
      EOS_ASSERT(bind.appid != 0, assert_exception, "zero appid");
      EOS_ASSERT(user_metadata.appid == 0, assert_exception, "already bind ${appid}",("appid",user_metadata.appid));
      auto app = db.find<app_object,by_appid>(bind.appid);
      EOS_ASSERT(app == nullptr, assert_exception, "app exists");
      db.create<app_object>([&]( auto& a ) {
         a.appid = bind.appid;
         a.owner = bind.account;
         a.expire = bind.expire;
         a.memo = bind.memo;
         a.created = context.control.pending_block_time();
      });
      db.modify( user_metadata, [&]( auto& a ) {
         a.appid = bind.appid;
      });
      context.add_ram_usage(bind.account, 1024);
   } else if (bind.opr_code == (uint32_t)bindapp::opr_code::modify) {
      const app_object *app = nullptr;
      if (bind.appid != 0) {
         app = db.find<app_object,by_appid>(bind.appid);
         if (app != nullptr && app->appid != user_metadata.appid)
            context.require_authorization(app->owner);
      } else if (user_metadata.appid != 0) {
         app = db.find<app_object,by_appid>(user_metadata.appid);
      }
      EOS_ASSERT(app != nullptr, assert_exception, "app not exists");
      db.modify( *app, [&]( auto& a ) {
         if (bind.has_expire())
            a.expire = bind.expire;
         if (bind.has_memo())
            a.memo = bind.memo;
      });
   } else if (bind.opr_code == (uint32_t)bindapp::opr_code::transfer) {
      EOS_ASSERT(bind.appid != 0, assert_exception, "appid zero");
      EOS_ASSERT(user_metadata.appid == 0, assert_exception, "already bind ${appid}",("appid",user_metadata.appid));
      auto app = db.find<app_object,by_appid>(bind.appid);
      EOS_ASSERT(app != nullptr, assert_exception, "app not exists");
      context.require_authorization(app->owner);
      auto& owner_metadata = db.get<account_metadata_object, by_name>(app->owner);
      context.add_ram_usage(bind.account, 1024);
      context.add_ram_usage(app->owner, -1024);
      db.modify( user_metadata, [&]( auto& a ) {
         a.appid = bind.appid;
      });
      db.modify( owner_metadata, [&]( auto& a ) {
         a.appid = 0;
      });
      db.modify( *app, [&]( auto& a ) {
         a.owner = bind.account;
...
static bool is_valid_sym(uint64_t value) {
   auto sym = value;
   for ( int i=0; i < 7; i++ ) {
...
void apply_eosio_bindsym(apply_context& context) {
   auto bind = context.get_action().data_as<bindsym>();
   try {
   context.require_authorization(config::app_account_name);
   context.require_authorization(bind.account);

   EOS_ASSERT(is_valid_sym(bind.sym), assert_exception,
         "invalid symbol ${sym}",("sym",bind.sym));
   if (bind.has_tollor()) {
      EOS_ASSERT(!bind.tollor || context.is_account(bind.tollor), assert_exception,
         "invalid tollor ${name}",("name",bind.tollor));
   }
   if (bind.has_issuer()) {
      EOS_ASSERT(!bind.issuer || context.is_account(bind.issuer), assert_exception,
         "invalid issuer ${name}",("name",bind.issuer));
   }
   if (bind.has_fee()) {
      EOS_ASSERT(bind.maxfee == 0 || bind.maxfee >= bind.minfee, assert_exception,
            "invalid fee ${minfee},${maxfee}",
            ("minfee", bind.minfee)("maxfee", bind.maxfee));
   }

   auto& db = context.db;
   auto& user_metadata = db.get<account_metadata_object, by_name>(bind.account);
   if (bind.opr_code == (uint32_t)bindsym::opr_code::modify) {
      auto sym = db.find<sym_object,by_sym>(bind.sym);
      EOS_ASSERT(sym != nullptr, assert_exception, "sym not exists");
      if (sym->appid != user_metadata.appid) {
         auto sym_owner = db.get<app_object,by_appid>(sym->appid);
         context.require_authorization(sym_owner.owner);
      }
      db.modify( *sym, [&]( auto& a ) {
         if (bind.has_tollor())
            a.tollor = bind.tollor;
         if (bind.has_issuer())
            a.issuer = bind.issuer;
         if (bind.has_fee()) {
            a.minval = bind.minval;
            a.minfee = bind.minfee;
            a.level = bind.level;
            a.delta = bind.delta;
            a.maxfee = bind.maxfee;
         }
      });
   } else if (bind.opr_code == (uint32_t)bindsym::opr_code::transfer) {
      auto sym = db.find<sym_object,by_sym>(bind.sym);
      EOS_ASSERT(sym != nullptr, assert_exception, "sym not exists");
      EOS_ASSERT(user_metadata.appid != 0, assert_exception, "appid zero");
      EOS_ASSERT(sym->appid != user_metadata.appid, assert_exception, "same app");
      auto sym_owner = db.get<app_object,by_appid>(sym->appid);
      context.require_authorization(sym_owner.owner);
      context.add_ram_usage(bind.account, 256);
      context.add_ram_usage(sym_owner.owner, -256);
      db.modify( *sym, [&]( auto& a ) {
         a.appid = user_metadata.appid;
         if (bind.has_tollor())
            a.tollor = bind.tollor;
         if (bind.has_issuer())
            a.issuer = bind.issuer;
...
void apply_eosio_setcode(apply_context& context) {
   ...
   EOS_ASSERT(account.is_can_set_contract(), invalid_permission, "can not set contract ${name}", ("name",act.account));
   //
...
void apply_eosio_setabi(apply_context& context) {
   ...
   EOS_ASSERT(account_metadata.is_can_set_contract(), invalid_permission, "can not set abi ${name}", ("name",act.account));
   //
