...
// by myextend
read_only::get_app_results read_only::get_app( const get_app_params& params )const {
   get_app_results result;
   auto& app = db.db().get<app_object,by_appid>(params.appid);
   result.appid = params.appid;
   result.owner = app.owner;
   result.memo = app.memo;
   result.expire = app.expire;
   result.created = app.created;
   return result;
}

read_only::get_sym_results read_only::get_sym( const get_sym_params& params )const {
   get_sym_results result;
   auto& sym = db.db().get<sym_object,by_sym>(symbol(0,params.sym.c_str()).to_symbol_code().value);
   result.sym = params.sym;
   result.appid = sym.appid;
   if (sym.tollor)
      result.tollor = sym.tollor;
   if (sym.issuer)
      result.issuer = sym.issuer;
   result.minval = sym.minval;
   result.minfee = sym.minfee;
   result.level = sym.level;
   result.delta = sym.delta;
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   // by myextend
   result.attrs       = accnt_metadata_obj.attrs;
   result.appid       = accnt_metadata_obj.appid;
   //
