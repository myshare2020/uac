...
public:
   ...
   // by myextend
   struct get_app_results {
      uint32_t                   appid;
      name                       owner;
      chain::time_point          created;
      chain::time_point          expire;
      string                     memo;
   };
   struct get_app_params {
      uint32_t           appid;
   };
   get_app_results get_app( const get_app_params& params )const;

   struct get_sym_results {
      string                     sym;
      uint32_t                   appid;
      optional<name>             tollor;
...
FC_REFLECT( eosio::chain_apis::read_only::get_account_results,
            ...
            (attrs)(appid) )
FC_REFLECT( eosio::chain_apis::read_only::get_app_results,
