...
extern "C" {
   ...
   uint64_t get_core_sym() {
      return intrinsics::get().call<intrinsics::get_core_sym>();
   }
   capi_name get_app_fee( uint64_t sym, int64_t value, int64_t* fee ) {
      return intrinsics::get().call<intrinsics::get_app_fee>(sym, value, fee);
   }
   void new_app_sym( uint64_t sym, uint64_t owner, uint64_t issuer ) {
      return intrinsics::get().call<intrinsics::new_app_sym>(sym, owner, issuer);
   }
   capi_name get_sym_issuer( uint64_t sym ) {
      return intrinsics::get().call<intrinsics::get_sym_issuer>(sym);
   }
   //
